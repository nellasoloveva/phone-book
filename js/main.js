      ...
      window.addEventListener('click', function(event) {
        if (event.target.id.startsWith('aye')) {
          ...
          let divInf = document.createElement('div');
            divInf.innerHTML = `<div class="n">Name: ${person.name}</div><div class="s">Surname: ${person.surname}</div><div class="pn">Phone number: ${person.phoneNumber}</div><div class="a">Adress: ${person.adress}</div>`
